...
plot_pdf.prob_1d_bma_forecast <- function(x, actual=NA, ymax=NA) {
  ...
  model <- get_discrete_continuous_model(x)
  ymax <- ifelse(is.na(ymax), max(model$dbeta)+0.5, ymax)

  g <- ggplot2::ggplot(data.frame(x=model$xseq, y=model$dbeta), mapping=aes(x=x, y=y)) +
  ...
  for (i in seq_len(dim(model$members$dbeta)[2])) {
    g <- g + ggplot2::geom_line(data.frame(x=model$xseq, y=model$members$dbeta[,i]), mapping=aes(x=x, y=y), col="black")
  }

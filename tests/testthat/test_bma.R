...
test_that("get_weighted_probability is correct, including clipping threshold", {
  ...
  with_mock(dbeta = function(obs, a, b) return(a + b),
            expect_equal(get_weighted_probability(OBS=0.99, PoC=0.4, param1=0.4, param2=0.01, w=0.5, percent_clipping_threshold=0.99, bma_distribution="beta", max_power=NA), 20)) # 0.5 * 0.4/0.01 = 20
  # Unclipped
  with_mock(dbeta =function(obs, a, b) return(a+b), # db = 0.01
            pbeta =function(obs, a, b) return(0.8),
    expect_equal(get_weighted_probability(OBS=0.989, PoC=0.4, param1=0.4, param2=0.01, w=0.5, percent_clipping_threshold=0.99, bma_distribution="beta", max_power=NA), 0.00375)) # 0.5 * 0.6 * 0.01/0.8 =0.00375
})

test_that("get_weighted_probability handles missing members", {
  expect_true(is.na(get_weighted_probability(OBS=NA, PoC=0.4, param1=0.4, param2=0.01, w=0.5, percent_clipping_threshold=0.99, bma_distribution="beta", max_power=NA)))
  expect_true(is.na(get_weighted_probability(OBS=0.8, PoC=NA, param1=0.4, param2=0.01, w=0.5, percent_clipping_threshold=0.99, bma_distribution="beta", max_power=NA)))
})
...
test_that("e_step array handling is correct.", {
  ...
  with_mock(get_rho= function(FCST, B0, B1, ...) return(sum(FCST, B0, B1)),
            ...
            get_gamma = function(rho, sigma) return(sum(rho, sigma)), # 3:2:17
            # 6, 11, 16, 21, 22, 27, 32, 37 ->  c(4, 7, 10, 13, 8, 11, 14, 17 ) -> c(14, 17, 20, 23, 28, 31, 34, 37)
            get_weighted_probability= function(OBS, PoC, param1, param2, w, percent_clipping_threshold, bma_distribution="beta", max_power=NA) return(sum(OBS, param1, param2)-PoC+OBS+w),
            OUT <- e_step(w, C0, OBS, FCST, B0, B1, PoC, B_transform=NA, percent_clipping_threshold=NA, bma_distribution="beta", max_power=NA))
...
test_that("e_step sumz handles NaN's.", {
  ...
  with_mock(get_rho= function(...) return(NA), get_gamma = function(...) return(NA),
            ...
            OUT <- e_step(w=NA, C0=NA, OBS=OBS, FCST=OBS, B0=NA, B1=NA, PoC=NA, B_transform=NA, percent_clipping_threshold=NA, bma_distribution="beta", max_power=NA))
...
test_that("em_subfunction is correct.", {
  with_mock(e_step=function(...) {return(list(z=(array(c(0.1, 0.2, 0.3, NA, seq(0.3, 0.9, by = 0.2), rep(NA, 4)), dim=c(2, 2, 3)))))},
            ...
            OUT <- em_subfunction(FCST=array(1:12, dim = c(2,2,3)), OBS=NA, PoC=NA, B0=NA, B1=NA, C0=4, w=c(0.2, 0.8, 0),
                                  percent_clipping_threshol=NA, bma_distribution = "beta", max_power=NA, count=1, CM2.iter=1))
...
test_that("beta1_ens_models throws errors", {
...
test_that("bma_ens_models coefficient data look-up is correct", {
  ...
  with_mock(get_lr= fake_lr, get_lm=fake_lm,
            ...
            OUT <- bma_ens_models(tel=c(0.25, 0.5, 0.75), ens=matrix(seq(0.1, 0.9, by=0.1), ncol=3))
  )
...
test_that("bma_ens_models coefficient data look-up handles missing fields", {
  fake_lr <- function(e, tel, ...) {
    coeffs <- c(sum(e), sum(tel))
    names(coeffs) <- c("(Intercept)", "x")
    probs <- c(e[1],e[3])
...
test_that("get_sigma calculation is correct", {
  # Example under the theoretical limit
  expect_equal(get_sigma(mu=0.7, C0=0.1), sqrt(0.084)) # sigma = sqrt(0.1-0.4*0.04), gamma = 0.21/0.984 - 1
  # Example over the theoretical limit
  expect_equal(get_sigma(mu=0.7, C0=0.25), sqrt(0.21)-1e-6) # sigma = sqrt(0.21)-1e-6
})

test_that("get_sigma handles NA's and mean on boundary", {
  expect_true(is.na(get_sigma(mu=NA, C0=0.1)))
  expect_true(is.na(get_sigma(mu=0, C0=0.1)))
})

test_that("get_gamma calculation is correct", {
  expect_equal(get_gamma(mu=0.7, sigma=sqrt(0.084)), 0.21/0.084 - 1) # sigma = sqrt(0.1-0.4*0.04), gamma = 0.21/0.984 - 1
})

test_that("get_gamma handles NA's and mean on boundary", {
  expect_true(is.na(get_gamma(mu=NA, sigma=0.1)))
  expect_true(is.na(get_gamma(mu=0, sigma=0.1)))
})
...
test_that("get_log_lik is correct",{
  with_mock(e_step=function(...) return(list(sumz=c(NA, exp(2), exp(5)))),
            expect_equal(get_log_lik(C0=NA, w=NA, OBS=NA, FCST=NA, B0=NA, B1=NA, PoC=NA, B_transform=NA, percent_clipping_threshold=NA, bma_distribution="beta", max_power=NA), 7)
  )
})
